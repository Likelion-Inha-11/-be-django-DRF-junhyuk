...
class category_post_create(APIView):
    def post(self,request):
        post = Post()
        post.headline = request.data["headline"]
...
class category_post_lookup(APIView):
...
class myposts_lookup(APIView):
    def get(self,request):
        posts = Post.objects.filter(writer = request.user)

        postSerializer = postSerializer(posts, many=True)
        return Response(postSerializer.data,status=200)
class mycomments_lookup(APIView):
    def get(self,request):
...
class userposts_lookup(APIView):
    def get(self, request):
...
class postslike(APIView):
    def post(self,request):
        post = Post.objects.get(id = request.data["id"])#DB에 해당 id를 가진 post 객체를 가져온다.

        if request.user in post.like.all():#req
